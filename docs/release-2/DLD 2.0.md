...
- Null/unknown ticket IDs → reject with message.
- Invalid state transitions → reject with message.
- Return with past travelDate → reject.
- Search with no matches → friendly message.
- Monetary formatting and rounding are simplistic; consider fixed-precision formatting.

---

## 10) Non-Functional Notes & Future Enhancements
- Replace linear search with indexed lookup (e.g., `unordered_map<int, Ticket>`).
- Externalize penalty policy (strategy/config) and currency formatting.
- Introduce reservation timeout to auto-release `BLOCKED` tickets.
- Persist data beyond process lifetime; introduce serialization or a DB.
- Replace `system("pause")` for portability.

---

## 11) Build & Run (reference)
- Single TU build (as in `main.cpp` includes) works for demo.
- Preferred: compile each `.cpp` and link; keep `SystemClock.hpp` header-only or move to `.cpp` if needed.

---

## 12) Appendix — Sample Console Outputs
- Search: list of AVAILABLE tickets to destination.
- Block/Sell/Return: status updates and refund line with penalty%.
- Report: totals and timestamp.

```text
[Cashier] Searching for available tickets to: Tallinn
Ticket #2 from Tartu to Tallinn | Price: 30 | Status: AVAILABLE
Ticket 2 BLOCKED.
Ticket must be BLOCKED before selling.
Ticket sold to: Kamil
Refund: 27.000000 (Penalty Rate: 10.000000%)
[Report for Cashier C1 - 2025-10-27 12:34:56]
Sold tickets: 1
Returned tickets: 1
```
